...
class MultiScaleEmbedding(nn.Module):
    def __init__(self, patch_configs, embed_dim):
        """
        ...
        for kernel_size, stride in patch_configs:
            # 對於每個 feature 單獨處理，conv1d 的 in_channels=1
            conv = nn.Conv1d(in_channels=1, out_channels=embed_dim,
    ...
    def forward(self, x):
        ...
        x: (B, T, F)
        process : 
          1. 先將 x 轉換成 (B, F, T) 再將其展平成 (B*F, 1, T)
        ...
        B, T, F = x.shape
        outputs = []
        # 轉換到 (B, F, T) 再展平 feature 到 batch 內
        x_permuted = x.permute(0, 2, 1)  # (B, F, T)
        x_reshaped = x_permuted.reshape(B * F, 1, T)  # (B*F, 1, T)
        for conv in self.patch_convs:
            ...
            outputs.append(out)
        # 沿 tokens 維度連接
        out = torch.cat(outputs, dim=1)  # (B,4 total_tokens, F, embed_dim)
        return out
...
# 5. Single step decoder : for the prediction adjustion of singlre time step
...
class MultiPatchFormer(nn.Module):
    def __init__(self, patch_configs, feature_shape, embed_dim,
    ...
    def forward(self, x):
        ...
        流程：
          ...
          4. 於 tokens 維度做平均，獲得 (B, F, embed_dim)
          5. single value解碼器：輸出 (B, out_dim) 
        """
        ...
        out = self.decoder(context)           # (B, out_dim) 
        return out

if __name__ == '__main__':
    # (batch, time, feature) = (64, 10, 16) the shape after the relevant feature embedding
    batch_size = 64
    T = 10
    F = 16
    x = torch.randn(batch_size, T, F)
    
    # 例如針對 T=10 設定 patch_configs 為 [(2,1), (3,1)]
    patch_configs = [(2, 1), (3, 1)]
    embed_dim = 32
    # decoder: patch_length=2, decoder_steps=4 -> 預測長度 T_pred = 2*4 = 8
    model = MultiPatchFormer(patch_configs=patch_configs,
                             feature_shape=16,
                             embed_dim=embed_dim,
                             temporal_layers=1,
                             temporal_heads=4,
