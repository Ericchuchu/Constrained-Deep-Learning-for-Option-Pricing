...
class DataProcessor:
    ...
    def compute_train_kde(self, train_input_convlstm: torch.Tensor):
        """Compute KDE model from training dataset's invm values."""
        # Extract required values from training data
        strike_price = train_input_convlstm[:, 0, -1, 4].numpy()
        underlying_price = train_input_convlstm[:, 2, -1, 2].numpy()
        sigma = train_input_convlstm[:, 0, -1, 3].numpy()
        t = train_input_convlstm[:, 0, -1, 2].numpy()  # time to maturity
        cp_flag = train_input_convlstm[:, 0, -1, 1].numpy()
        
        # Calculate invm for entire training dataset using vectorized operation depend on cp_flag
        invm = np.where(
            cp_flag == 0,
            -np.log(strike_price / underlying_price) / (sigma * np.sqrt(t)),
            np.log(strike_price / underlying_price) / (sigma * np.sqrt(t))
        )
                
        # Fit KDE model
        self.kde_model = gaussian_kde(invm.reshape(-1, 1).T, bw_method=0.5)
        weights = 1.0 / (self.kde_model(invm.reshape(-1, 1).T) + 1e-6)
...
def compute_pde_loss(V: torch.Tensor, t: torch.Tensor, S: torch.Tensor, sigma: torch.Tensor, r: float) -> float:
    try:
        # ∂V/∂t
        dV_dt = torch.autograd.grad(V.sum(), t, create_graph=True, retain_graph=True, allow_unused=True)[0]
        if dV_dt is None:
            dV_dt = torch.zeros_like(V)
        
        # ∂V/∂S
        dV_dS = torch.autograd.grad(V.sum(), S, create_graph=True, retain_graph=True, allow_unused=True)[0]
        if dV_dS is None:
            dV_dS = torch.zeros_like(V)

        #  ∂²V/∂S²
        d2V_dS2 = torch.autograd.grad(dV_dS.sum(), S, create_graph=True, retain_graph=True, allow_unused=True)[0]
        if d2V_dS2 is None:
            d2V_dS2 = torch.zeros_like(V)
        
        # PDE 殘差：∂V/∂t + 0.5 * σ² * S² * ∂²V/∂S² + r * S * ∂V/∂S - r * V
        residual = dV_dt + 0.5 * sigma**2 * S**2 * d2V_dS2 + r * S * dV_dS - r * V
        pde_loss = torch.mean(residual**2)
        return pde_loss
        
    except Exception as e:
        print(f"Warning: Error in PDE loss computation: {str(e)}")
        return torch.tensor(0.0, device=V.device)

def compute_losses(criterion, x_convlstm, input_tensor_convlsym_norm, outputs, y, kde_model, mean_weights):
    V, t, S = outputs
    sigma = x_convlstm[:, 0, -1, 3]
    r = 0.0079

    # unnormalize x_convlstm
    x_convlstm = input_tensor_convlsym_norm.unnormalize(x_convlstm)
    
    # Calculate invm for current batch
    strike_price = x_convlstm[:, 0, -1, 4].detach().cpu().numpy()
    underlying_price = x_convlstm[:, 2, -1, 2].detach().cpu().numpy()
    cp_flag = x_convlstm[:, 0, -1, 1].detach().cpu().numpy()
    sigma_np = x_convlstm[:, 0, -1, 3].detach().cpu().numpy()
    t_np = x_convlstm[:, 0, -1, 2].detach().cpu().numpy()

    # Calculate invm depending on cp_flag using numpy (vectorized)
    invm = np.where(
        cp_flag == 0,
        -np.log(strike_price / underlying_price) / (sigma_np * np.sqrt(t_np)),
        np.log(strike_price / underlying_price) / (sigma_np * np.sqrt(t_np))
    )
            
    # Fit KDE: invm is already a numpy array, so reshape as required
    weights = 1.0 / (kde_model(invm.reshape(1, -1)) + 1e-6)
    ...
    return mse_loss + lambdaa * pde_loss, mse_loss, pde_loss

def evaluate(model, data_loader, criterion, dict, device, last_epoch=False):
    model.eval()
    total_loss = 0
    with torch.no_grad():
        for x_convlstm, x_transformer, y, timestamp in data_loader:
            x_convlstm = x_convlstm.to(device).requires_grad_(True)
            x_transformer = x_transformer.to(device).requires_grad_(True)
            y = y.to(device).requires_grad_(True)
            
            outputs = model(x_convlstm, x_transformer)
            mse_loss = criterion(outputs[0], y)
...
def train_model(model: nn.Module, train_loader: data.DataLoader, valid_loader: data.DataLoader, test_loader: data.DataLoader, input_tensor_convlstm_norm: Normalization,
    ...
    for epoch in range(num_epochs):
        ...
        for x_convlstm, x_transformer, y, timestamp in tqdm(train_loader, desc=f'Epoch {epoch+1}/{num_epochs} - Training'):
            x_convlstm = x_convlstm.to(device).requires_grad_(True)
            x_transformer = x_transformer.to(device).requires_grad_(True)
            y = y.to(device).requires_grad_(True)
            ...
            optimizer.zero_grad()
            outputs = model(x_convlstm, x_transformer)
            adjusted_loss, mse_loss, pde_loss = compute_losses(criterion, x_convlstm, input_tensor_convlstm_norm, outputs, y, kde_model, mean_weights)
        ...
        if valid_loss < best_valid_loss:
            ...
            }, f"checkpoints/dualnetwork_best.pth")
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'loss_curves/loss_curves_dualnetwork_epoch{args.max_epoch}_early_stop{args.early_stop_mode}_syn{args.syn_data}.png')
    plt.close()
...
def test_model(model: nn.Module, data_loader: data.DataLoader, dataset_type: str, 
    ...
    plot_len = min(120, len(results['strike_price']))
    fig = plt.figure(figsize=(15, 10))
    ax = fig.add_subplot(111, projection='3d')
    
    # Normalize data for better visualization
    strike_prices = np.array(results['strike_price'][:plot_len])
    ...
    scatter1 = ax.scatter(strike_prices, time_to_maturity, true_prices, 
                         c='blue', marker='o', label='True Price', alpha=0.6)
    scatter2 = ax.scatter(strike_prices, time_to_maturity, estimated_prices, 
                         c='red', marker='^', label='Estimated Price', alpha=0.6)
    
    # Add error lines
    for i in range(plot_len):
        ax.plot([strike_prices[i], strike_prices[i]], 
    ...
    plt.savefig(f'testing_result/testing_result_dualnetwork_epoch{args.max_epoch}_early_stop{args.early_stop_mode}_syn{args.syn_data}.png', 
...
def plot_timestamp_loss_modified(train_loss_dict, valid_loss_dict, test_loss_dict, args):
    ...
    # Save the enhanced plot
    plt.savefig(f'loss_curves/timestamp_loss_curves_dualnetwork_epoch{args.max_epoch}_early_stop{args.early_stop_mode}_syn{args.syn_data}.png',
                dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    try:
        ...
        for dataset_type in ['train', 'test', 'valid']:
            ...
            raw_data[dataset_type] = (input_tensor_convlstm, input_tensor_transformer, label_tensor, timestamp_tensor)
        ...
        test_results_df = test_model(
            ...
            normalizations['test'][0], normalizations['test'][1], normalizations['test'][2],
            device, args
        )
        
        # Save results
        timestamp = datetime.now().strftime('%b%d_%H%M%S')
        results_path = f'{timestamp}_dual_network_results.csv'
